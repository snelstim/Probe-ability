    ...
    def start_cook(
        ...
        if not self._ambient_sensor_ok():
            raise HomeAssistantError(
                ...
                translation_key="ambient_unavailable",
            )
    ...
    def stop_cook(self, probe_index: int | None = None) -> None:
        ...
        if want_export or want_share:
            for i in indices:
                ...
                # Tolerance: a peak within probe accuracy of the target is a
                # completed cook, and retrain.py drops reached_target=false
                # exports — a strict compare would silently discard them.
                reached = peak_temp >= pred.target_temp - TARGET_REACHED_TOLERANCE_C

...
class CookMonitor:
    ...
    async def _async_export_csv(
        ...
        Data columns: elapsed_s, internal_temp_c, ambient_temp_c, predicted_remaining_s.

        predicted_remaining_s is generated by replaying the cook through a fresh
        predictor instance — it reproduces the exact estimates shown live on the
        card (None / empty during the initial collecting phase).
        """
        import csv
        import os
        ...
        def _write() -> None:
            ...
            with open(filepath, "w", newline="", encoding="utf-8") as fh:
                fh.write("# probe_ability_export_version: 2\n")
                ...
                writer.writerow(["elapsed_s", "internal_temp_c", "ambient_temp_c", "predicted_remaining_s"])
                for (ts, internal, ambient), remaining in zip(readings, predicted):
                    writer.writerow([
                        round(ts - t0, 1),
                        round(internal, 2),
                        round(ambient, 2),

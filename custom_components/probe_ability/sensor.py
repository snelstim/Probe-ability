...
class CookPredictorSensorBase(SensorEntity):
    ...
    def available(self) -> bool:
        """Available whenever the integration is loaded.

        All per-probe sensors mirror probe 0: always available, with state
        "unknown" (native_value None) while their probe is idle.  Staying
        available means HA keeps the attributes exposed — the card relies on
        attrs.probe_index to target the correct probe when a dashboard uses
        one card per probe (issues #4/#5).
        """
        return self._probe_index < len(self._monitor.predictors)
...
class CookTimeRemainingSensor(CookPredictorSensorBase):
    ...
    def __init__(self, monitor, entry: ConfigEntry, probe_index: int = 0) -> None:
        ...
        key_suffix = _PROBE_KEY_SUFFIX.get(probe_index, f"_probe{probe_index + 1}")
        self._attr_unique_id = f"{entry.entry_id}_time_remaining{suffix}"
        self._attr_translation_key = f"time_remaining{key_suffix}"
...
class CookETASensor(CookPredictorSensorBase):
    ...
    def __init__(self, monitor, entry: ConfigEntry, probe_index: int = 0) -> None:
        ...
        self._attr_translation_key = f"estimated_completion{key_suffix}"
